...
const Profile = () => {
    ...
    const [passwordCodeInput, setPasswordCodeInput] = useState("");
    ...
    const handleUpdateProfile = async (e: React.FormEvent) => {
        ...
        if (!nome || !sobrenome || !telefone || !estado || !cidade || !bairro || !logradouro || !numero || !cep) {
        ...
        try {
            ...
            await toast.promise(promise, {
                ...
                error: (err) => `Erro ao atualizar: ${err.response?.data?.erro || "tente novamente."}`
            });
    ...
    const handleRequestEmailChange = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await toast.promise(promise, {
                ...
                error: (err) => `Erro: ${err.response?.data?.erro || "tente novamente."}`
            });
            setEmailStep(2);
        } catch (error) {
            console.error(error);
    ...
    const handleRequestPasswordChange = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const promise = api.post("/users/request-password-change", { id: user!.id });
            await toast.promise(promise, {
                loading: "Enviando código...",
    ...
    const handleConfirmPasswordChange = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!passwordCodeInput || !newPasswordInput) {
                ...
                <div className={styles.dashboardContainer}>
                    ...
                    <div className={styles.rightColumn}>
                        <div className={styles.sectionBlock}>
                            ...
                            <div className={styles.infoGroup}>
                                <p><strong>Sobrenome:</strong> <span>{user.sobrenome || ""}</span></p>
                            </div>
                            <div className={styles.infoGroup}>
                                <p><strong>Email:</strong> <span>{user.email || ""}</span></p>
                            </div>
                            <div className={styles.infoGroup}>
                                <p><strong>Telefone:</strong> <span>{formatarTelefone(user.telefone || "")}</span></p>
                            </div>
                        </div>
                        ...
                        {user.endereco && (
                            <div className={styles.sectionBlock}>
                                ...
                                <div className={styles.infoGroup}>
                                    <p><strong>Rua e Número:</strong> <span>{user.endereco.logradouro}, {user.endereco.numero}</span></p>
                                </div>
                                <div className={styles.infoGroup}>
                                    <p><strong>Bairro:</strong> <span>{user.endereco.bairro}</span></p>
                                </div>
                                <div className={styles.infoGroup}>
                                    <p><strong>Cidade e Estado:</strong> <span>{user.endereco.cidade} - {user.endereco.estado}</span></p>
                                </div>
                                <div className={styles.infoGroup}>
                                    <p><strong>CEP:</strong> <span>{user.endereco.cep}</span></p>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div className={styles.editModalOverlay}>
                    <div className={styles.editModal}>
                        <div className={styles.editHeader}>
                            <h2>Editar Perfil</h2>
                            <button onClick={() => setIsEditing(false)} className={styles.closeBtn}>&times;</button>
                        </div>
                        <form onSubmit={handleUpdateProfile} className={styles.editForm}>
                            <div className={styles.inputGrid}>
                                ...
                                <div className={styles.inputWrapper}>
                                    ...
                                    <input type="text" inputMode="numeric" placeholder="Telefone" value={formatarTelefone(telefone)} onChange={(e) => setTelefone(e.target.value.replace(/\D/g, ""))} required />
                                </div>
                                <div className={styles.inputWrapper}>
                                    <label>CEP</label>
                                    <input type="text" inputMode="numeric" placeholder="CEP" value={formatarCep(cep)} onChange={(e) => setCep(e.target.value.replace(/\D/g, ""))} required />
                                </div>
                                <div className={styles.inputWrapper}>
                                    <label>Estado</label>
                                ...
                                <div className={styles.inputWrapper}>
                                    <label>Número</label>
                                    <input type="text" placeholder="Número" value={numero} onChange={(e) => setNumero(e.target.value.replace(/\D/g, ""))} required />
                                </div>
                            </div>
                ...
                <div className={styles.editModalOverlay}>
                    <div className={styles.editModal}>
                        <div className={styles.editHeader}>
                            ...
                            <button onClick={() => setIsEmailModalOpen(false)} className={styles.closeBtn}>&times;</button>
                        </div>
                        {emailStep === 1 ? (
                            <form onSubmit={handleRequestEmailChange} className={styles.editForm}>
                                <div className={styles.inputWrapper}>
                                    ...
                                    <input
                                        ...
                                        required
                                        style={{ width: "100%", height: "3rem", paddingLeft: "1rem", boxSizing: "border-box", border: "none" }}
                                    />
                                </div>
                                <button type="submit" className={styles.saveBtn}>Continuar</button>
                            </form>
                        ) : (
                            <form onSubmit={handleConfirmEmailChange} className={styles.editForm}>
                                <p style={{ fontSize: "0.95rem", color: "#444" }}>
                                    Um código de verificação foi enviado para o seu e-mail atual (<strong>{user.email}</strong>).
                                </p>
                                <div className={styles.inputWrapper}>
                                ...
                                <button type="submit" className={styles.saveBtn}>Confirmar e Alterar</button>
                            </form>
                        )}
                    </div>
                </div>
            ...
            {isPasswordModalOpen && (
                <div className={styles.editModalOverlay}>
                    <div className={styles.editModal}>
                        <div className={styles.editHeader}>
                            <h2>Alterar Senha</h2>
                            <button onClick={() => setIsPasswordModalOpen(false)} className={styles.closeBtn}>&times;</button>
                        </div>
                        {passwordStep === 1 ? (
                            <form onSubmit={handleRequestPasswordChange} className={styles.editForm}>
                                <p style={{ fontSize: "1rem", color: "#444", marginBottom: "10px" }}>
                                    Para alterar sua senha, precisamos confirmar sua identidade.
                                    Um código de verificação será enviado para o e-mail: <strong>{user.email}</strong>.
                                </p>
                                <button type="submit" className={styles.saveBtn}>Enviar Código</button>
                            </form>
                        ) : (
                            <form onSubmit={handleConfirmPasswordChange} className={styles.editForm}>
                                <div className={styles.inputWrapper}>
                                    <label>Código de Verificação</label>
                                    <input
                                        type="text"
                                        placeholder="Digite o código"
                                        value={passwordCodeInput}
                                        onChange={(e) => setPasswordCodeInput(e.target.value.replace(/\D/g, ""))}
                                        required
                                        style={{ width: "100%", height: "3rem", paddingLeft: "1rem", boxSizing: "border-box", border: "none" }}
                                    />
                                </div>
            ...
            {isDeleteModalOpen && (
                <div className={styles.editModalOverlay}>
                    <div className={styles.editModal}>
                        <div className={styles.editHeader}>
                            <h2>Deletar Conta</h2>
                            <button onClick={() => setIsDeleteModalOpen(false)} className={styles.closeBtn}>&times;</button>
                        </div>
                        <form onSubmit={handleDeleteAccount} className={styles.editForm}>
                            <p style={{ fontSize: "1rem", color: "#444", marginBottom: "10px" }}>
                                Tem certeza que deseja deletar sua conta? Esta ação é <strong>irreversível</strong>.
                            ...
                            <button type="submit" className={styles.saveBtn} style={{ backgroundColor: "#e74c3c" }}>Deletar Definitivamente</button>
                        </form>
                    </div>
                </div>
            )}
